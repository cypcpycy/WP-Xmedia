(function ($) {
  ...
  const requestError = (xhr, fallback) => xhr?.responseJSON?.data?.message || xhr?.responseJSON?.message || fallback;
    ...
    request({ action: 'mlm_qq_login_start', login_type: loginType }).done(function (response) {
      ...
      loginPollTimer = setInterval(function () {
		...
		request({ action: 'mlm_qq_login_poll', identifier: response.data.identifier, login_type: loginType }).done(function (poll) {
		  ...
		  if (poll.data?.message) $('#mlm-qq-state').text(poll.data.message);
		  if (poll.data?.done) { clearInterval(loginPollTimer); loginPollTimer = null; $button.prop('disabled', false).text(loginType === 'mobile' ? 'QQ 音乐客户端扫码' : 'QQ 网页扫码'); return; }
		  ...
		  clearInterval(loginPollTimer); loginPollTimer = null; $('#mlm-qq-state').text(requestError(xhr, 'QQ 登录请求失败，请重新登录')); $button.prop('disabled', false);
		});
      }, 2000);
	}).fail(xhr => { $('#mlm-qq-state').text(requestError(xhr, '二维码获取失败')); $button.prop('disabled', false).text(loginType === 'mobile' ? 'QQ 音乐客户端扫码' : 'QQ 网页扫码'); });
  });
